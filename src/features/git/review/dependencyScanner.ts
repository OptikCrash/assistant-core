import { ImportSpec } from "./types"; // adjust path if needed
...
export function extractExports(content: string): string[] {
    ...
    const namedRegex =
        /export\s+(?:declare\s+)?(?:default\s+)?(?:abstract\s+)?(?:class|function|interface|type|const|let|var|enum)\s+([A-Za-z0-9_]+)/g;
    ...
    while ((match = namedRegex.exec(content)) !== null) {
        if (match[1] && match[1] !== "default") results.add(match[1]);
    }
    ...
    while ((match = reExportRegex.exec(content)) !== null) {
        ...
        inner
            ...
            .forEach(part => {
                ...
                if (asMatch?.[1]) results.add(asMatch[1]);
                else results.add(part.replace(/\s+as\s+.*/, "").trim());
            });
    }
 ...
 */
export function extractImportSpecs(content: string): ImportSpec[] {
...
function parseImportClause(clause: string): string[] {
    ...
    if (rest) {
        if (rest.startsWith("{")) symbols.push(...parseNamedBindings(rest));
        else if (rest.startsWith("*")) symbols.push(rest); // "* as ns"
    }

    return symbols.map(s => s.trim()).filter(Boolean);
}

function parseNamedBindings(named: string): string[] {
    // "{ A, B as C }" -> ["A", "C"]
...
function splitOnFirstComma(text: string): [string, string?] {
    const idx = text.indexOf(",");
    if (idx === -1) return [text];
    return [text.slice(0, idx), text.slice(idx + 1)];
}

export function scanDependencies(content: string): DependencyInfo {
    return {
